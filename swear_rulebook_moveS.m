function legal_moves=swear_rulebook_moveS(current_status,Color,is_castling_legal)
...
for i=1:m,
    ...
    if strcmp(current_status(i).status,'Alive')&&(and(strcmp(current_status(i).color,Color),strcmp(current_status(i).piece,'Rook')))
      ...
      for j=1:m,
          if and((j~=i),strcmp(current_status(i).status,'Alive'))
    ...
    if strcmp(current_status(i).status,'Alive')&&(and(strcmp(current_status(i).color,Color),strcmp(current_status(i).piece,'King')))
        ...
        try
            legal_moves=[legal_moves;create_legal_move_king()];
        catch
            legal_moves=create_legal_move_king();
        end
    end
end
end